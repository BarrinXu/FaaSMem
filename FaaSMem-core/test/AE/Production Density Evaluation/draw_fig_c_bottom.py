...
for i in range(len(systems_values['baseline']['memory_avg'])):
    traces_memory_info.append((np.std(traces_fire_buckets[i]), systems_values['baseline']['memory_avg'][i],
                               systems_values['FaaSMem']['memory_avg'][i], list(traces_info.keys())[i],
                               np.average(traces_fire_buckets[i])))
traces_memory_info.sort()
print(traces_memory_info)
tmp_list = []
for entry in traces_memory_info:
    tmp_list.append(1 - entry[2] / entry[1])
    print(entry[0], entry[2], entry[2] / entry[1])
print(max(tmp_list), min(tmp_list))
x_memory = [entry[0] for entry in traces_memory_info]
...
print(max(y_density))
parameter = np.polyfit(np.log(x_memory), y_density, 1)
p = np.poly1d(parameter)
ax1.plot(x_memory, p(np.log(x_memory)), linewidth=3, ms=14, label='Trends', color=colors[2])
# ax1.set_ylabel('Production\nDensity')
ax1.set_xlabel('$\sigma$ of Req Intervals')
ax1.set_ylim(1)
plt.savefig('fig_c_bottom' + '.pdf', bbox_inches='tight')
