...
def test_to_one(function_name, loop_cnt, duration, upd_configs):
    ...
    suffix = 'idleHeat_' + function_name + '_' + str(loop_cnt) + '_' + str(duration) + '_' + str(
...
def test_to_all():
    print(input_args)
    general_configs = {'html_server': {'loop': 1, 'duration': 1,
                                       'configs': {'raw_memory': 354, 'exec_tuning': False, 'cpu': 0.2}},
                       'graph_bfs': {'loop': 1, 'duration': 1,
                                     'configs': {'raw_memory': 354, 'exec_tuning': False, 'cpu': 0.2}},
                       'recognizer_adult': {'loop': 1, 'duration': 1,
                                            'configs': {'raw_memory': 885, 'exec_tuning': False, 'cpu': 0.5}}}
    target_functions = {
        'graph_bfs': [[223, 223], [223, 128], [223, 64], [223, 32]],
        # 'html_server': [[240, 128], [240, 64]],
        # 'recognizer_adult': [[220, 64]],
    }
    for function_name in target_functions:
        memory_list = target_functions[function_name]
        now_configs = general_configs[function_name]
        for entry in memory_list:
            now_configs['configs'].update({'exec_memory': entry[0], 'idle_memory': entry[1]})
            test_to_one(function_name, now_configs['loop'], 60 * now_configs['duration'], now_configs['configs'])
