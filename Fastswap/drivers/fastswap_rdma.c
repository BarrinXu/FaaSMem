    ...
    struct rdma_cm_event *ev)
{
  ...
  case RDMA_CM_EVENT_DEVICE_REMOVAL:
    /* device removal ics handled via the ib_client API */
    break;
  default:
...
//  if (op == IB_WR_RDMA_READ){
//    msleep(10000);
//    pr_info("sswap_rdma_post_rdma: IB_WR_RDMA_READ start to do ib_post_send");
//  }
  ret = ib_post_send(q->qp, &rdma_wr.wr, &bad_wr);
//  if (op == IB_WR_RDMA_READ){
//
//    pr_info("sswap_rdma_post_rdma: IB_WR_RDMA_READ done ib_post_send, and start to wait 10000ms for async result");
//    msleep(10000);
//    pr_info("sswap_rdma_post_rdma: IB_WR_RDMA_READ done ib_post_send, and have waited 10000ms for async result");
//  }
  if (unlikely(ret)) {
...
int sswap_rdma_write(struct page *page, u64 roffset)
{
  ...
  int cpu;
  struct rdma_queue *q;
  // Todo: the get_cpu() pair, correct position ?
//  cpu = get_cpu();
  VM_BUG_ON_PAGE(!PageSwapCache(page), page);
  cpu = get_cpu();
  q = sswap_rdma_get_queue(cpu, QP_WRITE_SYNC);
  put_cpu();
...
int sswap_rdma_read_async(struct page *page, u64 roffset)
{
  ...
  // Todo: the get_cpu() pair, correct position ?
  cpu = get_cpu();
  q = sswap_rdma_get_queue(cpu, QP_READ_ASYNC);
  put_cpu();
