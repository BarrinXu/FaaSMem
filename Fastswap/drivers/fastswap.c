...
{
  pr_info("sswap_load start, CPU: %d\n", cpu);
...
{
  int ret;
  ret = frontswap_register_ops(&sswap_frontswap_ops);
  if (ret)
    pr_err("frontswap_register_ops failed\n");
